"""
ShoppingPlan / PlanItem / PurchaseRecord / PlanShare 模型
"""
...
class PlanSharePermission(str, enum.Enum):
    view = "view"
    edit = "edit"
...
class ShoppingPlan(Base):
    ...
    records = relationship(
        "PurchaseRecord", back_populates="plan", cascade="all, delete-orphan"
    )
    shares = relationship(
        "PlanShare", back_populates="plan", cascade="all, delete-orphan"
    )
...
class PlanShare(Base):
    ...
    permission = Column(
        Enum(PlanSharePermission), nullable=False, default=PlanSharePermission.view
    )
    created_at = Column(DateTime, default=datetime.utcnow)

    plan = relationship("ShoppingPlan", back_populates="shares")
