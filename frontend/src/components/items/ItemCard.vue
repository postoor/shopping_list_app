  ...
  <div class="card hover:shadow-md transition-shadow group">
    ...
    <div class="flex items-start justify-between mb-3">
      <div class="flex-1 min-w-0 pr-2">
        ...
        <p v-if="item.brand_note" class="text-xs text-gray-400 mt-0.5 truncate">
          {{ item.brand_note }}
        </p>
      </div>
      <!-- Actions -->
    ...
    <div class="flex items-center gap-4 text-sm text-gray-600 mb-4">
      <span
        >數量：<strong>{{ item.quantity }}</strong></span
      >
      <span v-if="item.est_price"
        >預估：<strong>NT$ {{ item.est_price }}</strong></span
      >
    </div>

    <p v-if="item.note" class="text-xs text-gray-400 mb-4 line-clamp-2">
      {{ item.note }}
    </p>
