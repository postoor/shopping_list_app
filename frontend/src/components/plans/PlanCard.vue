...
const emit = defineEmits(["toggle", "complete", "delete", "share"]);
...
<template>
  <div class="card">
    ...
    <div class="flex items-start justify-between mb-3">
      ...
      <div class="flex items-center gap-1">
        <button
          v-if="!plan.is_shared"
          @click="$emit('share')"
          class="text-xs text-blue-500 hover:text-blue-700 px-2 py-1 rounded hover:bg-blue-50 transition"
        >
          分享
        </button>
        <button
          v-if="plan.status !== 'completed' && !plan.is_shared"
          @click="$emit('delete')"
    ...
    <ul class="space-y-2 mb-4">
      ...
      >
        <input
          ...
          @change="
            $emit('toggle', { planId: plan.id, piId: pi.id, done: !pi.is_done })
          "
          class="w-4 h-4 rounded accent-primary-500 cursor-pointer"
        />
        <span
          :class="pi.is_done ? 'line-through text-gray-400' : 'text-gray-700'"
        >
          {{ itemName(pi.item_id) }}
        </span>
      </li>
    ...
    <p
      v-if="plan.status !== 'completed' && progress.pct < 100"
