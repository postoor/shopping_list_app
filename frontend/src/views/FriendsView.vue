...
async function sendInvite() {
  ...
  try {
    ...
    message.value  = { type: 'success', text: `邀請已發送至 ${inviteEmail.value}` }
    inviteEmail.value = ''
  } catch (e) {
    message.value = { type: 'error', text: e.response?.data?.detail || '發送失敗' }
  } finally {
    sending.value = false
  }
}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="card">
        ...
        <form @submit.prevent="sendInvite" class="flex gap-2">
          <input v-model="inviteEmail" type="email" required class="input" placeholder="friend@example.com" />
          <button type="submit" class="btn-primary shrink-0" :disabled="sending">
            {{ sending ? '發送中…' : '發送邀請' }}
          </button>
        </form>
        ...
        <div class="mt-5">
          ...
          <div v-if="store.invitations.length === 0" class="text-sm text-gray-400">尚無邀請記錄</div>
          <ul v-else class="space-y-2">
            <li v-for="inv in store.invitations" :key="inv.id"
              ...
              <span class="text-xs px-2 py-0.5 rounded-full"
                :class="inv.is_used ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'">
                {{ inv.is_used ? '已接受' : '待接受' }}
              </span>
            </li>
          </ul>
      ...
      <!-- 好友清單 -->
      <div class="card">
        <h2 class="text-base font-semibold text-gray-900 mb-4">👥 好友清單（{{ store.friends.length }}）</h2>
        <div v-if="store.loading" class="text-sm text-gray-400">載入中…</div>
        <div v-else-if="store.friends.length === 0" class="text-center py-8 text-gray-400">
          <div class="text-3xl mb-2">👥</div>
          <p class="text-sm">尚無好友，發送邀請開始協作吧！</p>
        </div>
        <ul v-else class="space-y-2">
          <li v-for="friend in store.friends" :key="friend.id"
            class="flex items-center justify-between p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition">
            <div class="flex items-center gap-3">
              <div class="w-9 h-9 rounded-full bg-primary-100 text-primary-700 font-semibold flex items-center justify-center text-sm">
            ...
            <button @click="store.removeFriend(friend.id)"
