...
async def list_items(
    ...
    my_items = (await db.execute(
        select(Item).where(Item.owner_id == me.id)
    )).scalars().all()
...
async def get_item(
    ...
    if item.owner_id != me.id:
        share = (await db.execute(
            select(ItemShare).where(ItemShare.item_id == item.id, ItemShare.shared_with == me.id)
        )).scalar_one_or_none()
        if not share:
...
async def share_item(
    ...
    friendship = (await db.execute(
        select(Friendship).where(
            or_(
                and_(Friendship.requester_id == me.id,             Friendship.addressee_id == body.shared_with),
                and_(Friendship.requester_id == body.shared_with,  Friendship.addressee_id == me.id),
            )
        )
    )).scalar_one_or_none()
    if not friendship:
...
async def revoke_share(
    ...
    result = await db.execute(select(ItemShare).where(ItemShare.id == share_id, ItemShare.item_id == item_id))
