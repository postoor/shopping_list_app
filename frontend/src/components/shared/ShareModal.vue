...
onMounted(async () => {
  const { data } = await itemsApi.listShares(props.item.id)
  shares.value = data
})
...
async function addShare() {
  ...
  loading.value = true
  try {
    const { data } = await itemsApi.share(props.item.id, {
...
async function revokeShare(shareId) {
  await itemsApi.revokeShare(props.item.id, shareId)
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
      <div class="p-6">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-lg font-semibold text-gray-900">🔗 分享「{{ item.name }}」</h2>
        ...
        <div class="flex gap-2 mb-5">
          ...
          <button @click="addShare" class="btn-primary shrink-0" :disabled="!selectedFriend || loading">分享</button>
        </div>

        <!-- Current Shares -->
        <div>
          <p class="text-xs font-medium text-gray-500 mb-2">已分享對象</p>
          <div v-if="shares.length === 0" class="text-sm text-gray-400">尚未分享給任何人</div>
          <ul v-else class="space-y-2">
            <li v-for="share in shares" :key="share.id"
              ...
              <span class="text-gray-700">{{ friendName(share.shared_with) }}</span>
              <div class="flex items-center gap-2">
                <span class="text-xs text-gray-400 capitalize">{{ share.permission === 'view' ? '查看' : '編輯' }}</span>
                <button @click="revokeShare(share.id)" class="text-xs text-red-400 hover:text-red-600">撤銷</button>
              </div>
            </li>
          </ul>
        </div>
